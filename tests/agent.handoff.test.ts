...
describe('parseHandoffParams', () => {
  ...
  it('round-trips a URL built by the agent layer', () => {
    const url = buildHandoffUrl('https://ghostname.test/', { name: 'name.eth', chainId: 1, reportId: REPORT });
    const parsed = parseHandoffParams(paramsOf(url));
    expect(parsed).toEqual({
      ok: true,
      params: { source: 'agent', name: 'name.eth', chainId: 1, reportId: REPORT, version: 1 },
      ignored: [],
    });
  });

  it('normalizes the name and rejects invalid names, chains and versions', () => {
    expect(parseHandoffParams(new URLSearchParams('source=agent&name=Name.ETH&chainId=11155111&version=1'))).toMatchObject({
      ok: true,
      params: { name: 'name.eth', chainId: 11155111 },
    });
