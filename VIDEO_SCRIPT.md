...
| GitHub push credentials | Old machine's git or `gh` login | Only for `npm run deploy:pages`. | None | Not needed. The deployed app (gh-pages commit `a897a02`, built from source `cc0f15af1946`) is already current. |
...
Expected: the last line shows the current tip of `main`. At the time of writing that is `cc0f15a Merge pull request #8: Recording guide for a fresh laptop, design-ready slide copy, and guard support for new demo identities`. [Re-check on recording day; at review time `main` was `cc0f15a` and gh-pages `a897a02` built from `cc0f15af1946`. The deployed app's footer shows the commit the served bundle was built from.]
...
- [ ] Footer ends with "Writes go to Sepolia testnet. Build cc0f15af1946." [Re-check on recording day; at review time `main` was `cc0f15a` and gh-pages `a897a02` built from `cc0f15af1946`.]
...
Deployed app (hash routes, deep-linkable; gh-pages commit `a897a02`, built from source `cc0f15af1946`, footer reads "Writes go to Sepolia testnet. Build cc0f15af1946."):
...
Repository: https://github.com/0xSkrillah/ghostname (`main` at `cc0f15a` at the time of writing).
