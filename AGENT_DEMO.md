...
Check the built CLI works against live chain data (no keys needed):
...
> Audit skrillah.eth and help me improve its privacy.
...
Expected live result for `skrillah.eth` on chain 1: **INCOMPLETE**.
...
```text
https://0xskrillah.github.io/ghostname/#/create?name=skrillah.eth&chainId=1&source=agent&reportId=gcr1_...&version=1
```
...
Do not publish for `skrillah.eth`: mainnet writes are blocked in the shipped
...
```bash
node dist-agent/ghostname.mjs audit skrillah.eth --chain 1
node dist-agent/ghostname.mjs plan skrillah.eth --chain 1
